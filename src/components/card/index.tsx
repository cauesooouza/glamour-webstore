...
export default function Card({ title, price, image, rating }: cardInterface) {
    const RateImg = ({ repeatCount }: rateImg) => {
        ...
        for (let i = 0; i < Math.floor(repeatCount); i++) {
            images.push(
                <Star color='rgb(250 204 21)' fill='yellow' />
            );
        }

        if (repeatCount - Math.floor(repeatCount) > 0.4) {
            images.push(
                <StarHalf color='rgb(250 204 21)' fill='yellow' />
            )
    ...
    console.log(rating)
    return (
        <div className='w-[240px] lg:w-[280px] xl:w-[300px] flex flex-col rounded-lg border-[1px] border-gray-200 bg-white p-2'>
