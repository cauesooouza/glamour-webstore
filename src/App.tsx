        ...
        <section className="flex flex-col">
          <div className="container flex flex-col md:flex-row gap-6 justify-between pt-6 px-4">
            <div className="flex flex-col gap-3 md:justify-center">
              <h1 className="font-anton text-[2.75rem] leading-[3rem] md:text-5xl">Encontre produtos especiais para você</h1>
              <p className=" text-sm text-gray-600 md:">Navegue por nossa diversificada linha de produtos meticulosamente elaborados, projetadas para realçar sua individualidade e atender ao seu senso de estilo.</p>
              {/* compre agora */}
              <span className="inline-block w-full cursor-pointer hover:no-underline sm:max-w-[250px] py-3 rounded-full bg-black text-white text-center">compre agora</span>
              {/* about */}
              <div className="flex justify-evenly md:justify-start flex-wrap gap-x-8 gap-y-4 mb-4">
                ...
                {/* box */}
                <div className="flex flex-col">
                  <span className="font-bold text-lg md:text-2xl">+2.000</span>
                  <p className="text-gray-600 text-xs font-sans">Produtos de alta qualidade </p>
                </div>
                {/* box */}
                <div className="flex flex-col">
                  <span className="font-bold text-lg md:text-2xl">+30.000</span>
                  <p className="text-gray-600 text-xs font-sans">Clientes felizes</p>
                </div>
              </div>
            </div>
            <img src={modeloImg} alt="modelos vestindo roupas estilosas e ousando oculos" className="object-contain md:w-80 relative md:-right-4 self-end -z-10" />
          </div>
          {/* strip */}
        ...
        <section className='container flex flex-col items-center py-10'>
          ...
          <div className='container flex flex-wrap gap-6 justify-center'>
            {loading ? <LoadingComponent /> : products.map((product, index) => (
              <Card key={index} title={product.title} price={product.price} image={product.image} rating={product.rating} />
            )) }
